...
class NVR_Gcell
{
  public:
    ...
    void set_capacity(unsigned cap) {m_capacity = cap;}
  private:
    unsigned m_demand : 16;
    unsigned m_capacity : 16;
};
...
bool NVR_DB::read_files(int argc, char* argv[]) // argc = 4, argv[0] = ./evaluator, argv[1] = resource_file, argv[2] = net_file, argv[3] = GR_file
{
  if (argc < 4) {
    printf("%d\n", argc);
...
bool NVR_DB::read_gr_solution(const char *input)
{
  ...
  while(std::getline(fin, line)) {
    ...
    if(!net) {
      ...
      std::cout << xl << " " << yl << " " << zl << " " << xh << " " << yh << " " << zh << std::endl;
      //printf("(%d, %d, %d) (%d, %d, %d)\n", xl, yl, zl, xh, yh, zh);
      if(zh != zl) { // via
        if(xh == xl && yh == yl) {
  ...
  for(unsigned z = 0; z < m_graph.num_layer(); z++) {
    ...
    for(unsigned x = 0; x < m_graph.num_gridx(); x++) {
      for(unsigned y = 0; y < m_graph.num_gridy(); y++) {
        ...
        if(cell.demand() > 2 * cell.capacity()) {
          num_overflows += cell.demand() - 2 * cell.capacity();
        }

        if (cell.capacity() > 0 ){
          overflow = double(cell.demand()) - 2 * double(cell.capacity());
    ...
    printf("Layer = %d, num_overflows = %llu, layer_overflows = %lf, overflow cost = %lf\n", z, num_overflows, layer_overflows, overflow_cost);
  }
